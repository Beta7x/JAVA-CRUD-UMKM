...
public class MainActivity extends javax.swing.JFrame {
    ...
    public MainActivity() {
        ...
        Object[] header = {"NO"," ","Nama UMKM","Jenis UMKM","Bidang UMKM", "Pemilik UMKM", "Alamat UMKM", "Kota/Kabupaten"};
        ...
        DataTabel.getColumnModel().getColumn(1).setMaxWidth(0);
    ...
    private void ExportPDFMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ExportPDFMousePressed
        ...
        try {
            PdfWriter.getInstance(doc, new FileOutputStream(path + "/dataUmkm.pdf"));
            ...
            //add head
            tbl.addCell("NO");
            tbl.addCell("Nama UMKM");
            tbl.addCell("Jenis UMKM");
            tbl.addCell("Bidang UMKM");
            tbl.addCell("Pemilik UMKM");
            tbl.addCell("Alamat UMKM");
            tbl.addCell("Kota/Kabupaten");
            
            for(int i =0; i<DataTabel.getRowCount();i++){
                
                String NO = DataTabel.getValueAt(i, 0).toString();
                String NamaUmkm = DataTabel.getValueAt(i, 2).toString();
                String JenisUmkm = DataTabel.getValueAt(i, 3).toString();
                String BidangUmkm = DataTabel.getValueAt(i, 4).toString();
                String PemilikUmkm = DataTabel.getValueAt(i, 5).toString();
                String AlamatUmkm = DataTabel.getValueAt(i, 6).toString();
                String KotaKabupaten = DataTabel.getValueAt(i, 7).toString();
                
                tbl.addCell(NO);
                tbl.addCell(NamaUmkm);
                tbl.addCell(JenisUmkm);
                tbl.addCell(BidangUmkm);
                tbl.addCell(PemilikUmkm);
                tbl.addCell(AlamatUmkm);
                tbl.addCell(KotaKabupaten);
...
//    }
    ...
    private App.Card card3;
    private App.Card card4;
    private App.Card card5;
    ...
    public void loadData(String key) {
        try {
            ...
            Connection c = Connect.MySQl();
            Statement st = c.createStatement();            
            String where = key.isEmpty()?"":"WHERE nama_umkm LIKE '%"+key+"%' OR jenis_umkm LIKE '%"+key+"%' OR bidang_umkm LIKE '%"+key+"%' OR pemilik_umkm LIKE '%"+key+"%' OR alamat_umkm LIKE '%"+key+"%' OR kota_kabupaten LIKE '%"+key+"%' ";
            
            ResultSet rs = st.executeQuery("SELECT * FROM umkm " + where);
            int no = 0;
            int kab = 0;
            int kota = 0;
            while (rs.next()) {                
                no++;
                int id = rs.getInt("id_umkm");
                String nama_umkm = rs.getString("nama_umkm");
                String jenis_umkm = rs.getString("jenis_umkm");
                String bidang_umkm = rs.getString("bidang_umkm");
                String pemilik = rs.getString("pemilik_umkm");
                String alamat = rs.getString("alamat_umkm");
                String kota_kab = rs.getString("kota_kabupaten");
                Object[] data = {no,id,nama_umkm,jenis_umkm,bidang_umkm,pemilik,alamat,kota_kab};
                int jumlah = data.length-1;
                model.addRow(data);
                
                if(kota_kab.equals("Kabupaten Tegal")) {
                    kab++;
                } else if(kota_kab.equals("Kota Tegal")) {
                    kota++;
                }
            }
            jmlhUmkm.setText("" + no);
            jmlhUmkmKabTegal.setText("" + kab);
            jmlhUmkmKotaTegal.setText("" + kota);
